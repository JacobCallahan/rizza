...
class Main:
    ...
    def __init__(self):
        ...
        parser.add_argument(
            "action",
            type=str,
            choices=["brute", "genetic", "config", "list", "test"],
    ...
    def brute(self):
        ...
        parser.add_argument(
            "-o", "--output-path", type=str, help="The file path to write the test tasks to."
        )
        parser.add_argument(
            "-i", "--import-path", type=str, help="The file path to exported test tasks."
        )
        parser.add_argument(
            "-l",
            "--log-name",
        ...
        parser.add_argument(
            "--max-fields", type=int, help="The maximum number of entity fields to use."
        )
        parser.add_argument(
            "--max-inputs", type=int, help="The maximum number of input methods to use."
        )
        parser.add_argument(
            "--field-exclude",
            type=str,
        ...
        parser.add_argument("--run-async", action="store_true", help="Run tests asynchronously.")
        parser.add_argument(
            "--async-limit",
            type=int,
            default=100,
        ...
        parser.add_argument("--debug", action="store_true", help="Enable debug loggin level.")

        args = parser.parse_args(sys.argv[2:])
        self.conf.load_cli_args(args, command=True)
        self.conf.init_logger(
            path=self.conf.base_dir.joinpath(f"logs/brute/{args.log_name}"),
        ...
        if args.import_path:
            ...
            for entity in args.entities:
                ...
                e_tester.prep(field_exclude=args.field_exclude, method_exclude=args.method_exclude)
                tests = e_tester.brute_force(
                    max_fields=args.max_fields, max_inputs=args.max_inputs
                )
                if args.output_path:
                    TaskManager.export_tasks(path=Path(args.output_path), tasks=tests)
                elif args.run_async:
    ...
    def genetic(self):
        ...
        parser.add_argument(
            "--fresh", action="store_true", help="Don't attempt to load in saved results."
        )
        parser.add_argument(
            "--prune",
            action="store_true",
        ...
        if args.prune:
            ...
            self.conf.init_logger(
                path=self.conf.base_dir.joinpath(f"logs/genetic/{gtester.test_name}.log"),
                level="debug" if args.debug else None,
            )
            gtester.run()
        else:
    ...
    def config(self):
        ...
        subparsers = parser.add_subparsers(
            dest="project", help="The component's config you want to change or view."
        )
        nailgun_config = subparsers.add_parser("nailgun")
        nailgun_config.add_argument("-u", "--user", type=str, help="Username")
        nailgun_config.add_argument("-p", "--password", type=str, help="Password")
        nailgun_config.add_argument(
            "-t",
            "--target",
            type=str,
        ...
        nailgun_config.add_argument(
            "--label", type=str, default="default", help="The configuration label to use."
        )
        nailgun_config.add_argument("--path", type=str, help="The configuration file path to use.")
        nailgun_config.add_argument(
            "--clear", action="store_true", help="Clear existing configuration."
        )
        nailgun_config.add_argument(
            "--show", action="store_true", help="Show existing configuration."
        )

        rizza_config = subparsers.add_parser("rizza")
        rizza_config.add_argument("--path", type=str, help="The configuration file path to use.")
        rizza_config.add_argument(
            "--clear", action="store_true", help="Clear existing configuration."
        )
        rizza_config.add_argument(
            "--show", action="store_true", help="Show existing configuration."
    ...
    def list(self):
        ...
        parser = argparse.ArgumentParser()
        parser.add_argument(
            "subject", type=str, choices=["entities", "methods", "fields", "args", "input-methods"]
        )
        parser.add_argument(
    ...
    def test(self):
        ...
        parser = argparse.ArgumentParser()
        parser.add_argument(
            "--args", type=str, nargs="+", help='pytest args to pass in. (--args="-r a")'
        )
        args = parser.parse_args(sys.argv[2:])
        pyargs = args.args if args.args else ["-q"]
