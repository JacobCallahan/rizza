...
def test_positive_create():
    ...
    assert gen_test.max_generations == gen_test.config.RIZZA["GENETICS"]["MAX GENERATIONS"]
    assert gen_test.population_count == gen_test.config.RIZZA["GENETICS"]["POPULATION COUNT"]
...
def test_positive_config_overrides():
    ...
    assert gen_test.config.RIZZA["GENETICS"]["MAX RECURSIVE GENERATIONS"] == RECURSE_LIMIT
    assert not gen_test.config.RIZZA["GENETICS"]["ALLOW RECURSION"]
    assert gen_test.config.RIZZA["GENETICS"]["MAX RECURSIVE DEPTH"] == RECURSE_LIMIT
